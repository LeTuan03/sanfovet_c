...
export default function Header() {
  ...
  return (
    <div className="w-full z-50 relative" suppressHydrationWarning>
      ...
      <div className="bg-gradient-to-r from-biotechvet-dark to-[#0d2b10] text-white text-[0.73rem] py-1.5 hidden md:block border-b border-white/5" suppressHydrationWarning>
          ...
          <div className="flex justify-between items-center" suppressHydrationWarning>
            ...
            <Link href="/" className="shrink-0 flex items-center gap-3">
              ...
              <div className="hidden lg:block">
                <div className="text-primary font-black text-xl leading-none tracking-tighter italic uppercase font-montserrat">Công ty CP Công nghệ Sinh học Thú y</div>
            ...
            <nav className="hidden lg:flex items-center gap-1">
                ...
                .map((menu) => {
                  ...
                  if (menu.hasMega) {
                    return (
                      <div key={menu.id} className="relative group p-0">
                        <Link href={menu.link} className="px-2 py-2 text-[0.73rem] font-black text-biotechvet-dark hover:text-primary uppercase tracking-tight flex items-center gap-1 group-hover:bg-biotechvet-alt rounded-lg transition-colors font-montserrat">
                          ...
                          <div className="px-2 space-y-0.5">
                            {children.map((child) => (
                              ...
                              >
                                ...
                                <span className="text-[0.73rem] font-bold text-gray-700 leading-snug group-hover/subitem:text-primary transition-colors">
                                  {t(child.name)}
                                </span>
                              </Link>
                            ))}
                  ...
                  if (menu.isButton) {
                    return (
                      <Link 
                        key={menu.id}
                        href={menu.link} 
                        className="ml-4 px-6 py-2.5 bg-primary text-white text-[0.73rem] font-black rounded-full hover:bg-primary-dark transition-all uppercase tracking-widest shadow-lg shadow-primary/20 active:scale-95 font-montserrat"
                      >
                        {t(menu.name)}
                      </Link>
                    );
                  ...
                  return (
                    <Link 
                      key={menu.id}
                      href={menu.link} 
                      className="px-2 py-2 text-[0.73rem] font-black text-biotechvet-dark hover:text-primary uppercase tracking-tight font-montserrat"
                    >
                      {t(menu.name)}
                    </Link>
                  );
              ...
              >
                <input
                  ...
                  className="pl-4 pr-10 py-1.5 w-[140px] lg:w-[180px] text-[0.73rem] border border-primary rounded-full focus:outline-none focus:ring-1 focus:ring-primary/50 transition-all placeholder:text-gray-400 font-montserrat"
                />
...
function DropdownItem({ href, icon, label }: { href: string, icon: React.ReactNode, label: string }) {
  return (
    <Link href={href} className="flex items-center gap-4 px-4 py-3 hover:bg-biotechvet-alt rounded-xl transition-all group/item">
       ...
       <span className="text-[0.73rem] font-bold text-gray-700 leading-snug group-hover/item:text-primary transition-colors">{label}</span>
    </Link>
  );
}
